...
echo BUIDE: '%BUIDE%'
IF NOT '%BUIDE%'=='' (
		   ECHO Run auto test... " /D'%RUN_PROJECT_LOCATION%' /WAIT /B %ScriptDirTest%\Jenkins_runRFT.bat '%RFT_PROJECT_LOCATION%' '%RFT_SCRIPT_NAME%' '%BUIDE%' '%isUnicode%' '%EMAIL_REPORT%' "OOOOO%RUN_PROJECT_LOCATION% CDOMessage.exe %Subject%,%EmailParameters%OOOOO" '%TEST_CATEGORY%' '%SILENT%'
	       CALL %ScriptDirTest%\Jenkins_runRFT.bat %RFT_PROJECT_LOCATION% %RFT_SCRIPT_NAME% %BUIDE% %isUnicode% %EMAIL_REPORT% "OOOOO%RUN_PROJECT_LOCATION% CDOMessage.exe %Subject%,%EmailParameters%OOOOO" %TEST_CATEGORY% %SILENT%
		   CALL :SEND_MAIL
		   )
GOTO :EOF
